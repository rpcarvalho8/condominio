...
1. **Σ permilagem_centesimas = 100000 (1000,00‰) — necessário mas não suficiente.** A permilagem canónica é um inteiro em centésimas de ‰, lido do token documental (`39,50` → 3950), sem arredondamento. Mais de 2 casas decimais no valor em ‰ deixa a centésima vazia e a linha por decidir. A coluna inteira `permilagem` só se grava quando o valor é um ‰ inteiro exacto (`centesimas % 100 === 0`); não entra na soma nem no rateio. A confirmação do admin (F1) tem de ser linha a linha, com o excerto do PDF de origem visível junto a cada fração — trocar permilagem entre duas frações mantém a soma mas vicia a cobrança. Se a soma não fecha em 100000, a confirmação falha; não se distribui resto para alterar o valor de uma unidade. O rateio de cêntimos de uma rubrica (F2) ordena as fracções por `codigo` ASC com a colação BINARY do SQLite (comparação de bytes, sem NOCASE: dígitos, depois maiúsculas, depois minúsculas; por exemplo `10`, `1A`, `9`, `AA`, `B`, `a`): cada uma, excepto a última, recebe `floor(amountCents * permilagem_centesimas / 100000)`; o resto inteiro de cêntimos vai todo para a última. Não é largest-remainder.
