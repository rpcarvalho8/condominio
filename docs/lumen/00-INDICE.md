...
Este pacote é **só documentação**. Nenhum código de orquestra (F6) antes de existirem tenant + ingestão (F0/F1) estáveis.

---

## Regra de precedência

Se dois documentos contradizerem um ao outro, a ordem de autoridade é:

**ADR-LOG (aceite) > 02-DOMINIO > 06-FATIAS > diagramas de arquitetura > resto**

O ADR-LOG diz *o que foi decidido e porquê*. Os outros ficheiros descrevem *como isso se manifesta*. Em caso de dúvida, não se inventa uma terceira regra — segue-se a precedência.
...
Resumo: congelar a documentação v6.1 → implementar F0 e F1 → F2 → piloto Essencial (F0–F3) → F5 + gates → comercial geral. Sem atalhos para dinheiro/votos/legal em produção.

---

## Índice do pacote (o que existe aqui)
