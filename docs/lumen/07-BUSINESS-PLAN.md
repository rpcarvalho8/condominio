...
**LUMEN** (nome anterior: BuildingMind) não se vende como "mais uma app de gestão de condomínio com IA" — isso copia-se assim que um concorrente acrescenta um chatbot. O posicionamento interno e a visão de arquitetura é:
...
**Moat = valor histórico, não prisão de dados.** Quanto mais tempo um condomínio usa o LUMEN, mais valioso fica o histórico — mas a alternativa a sair não pode ser "perder tudo". O produto prevê **Tenant Exit / portabilidade** (exportação estruturada do estado do tenant: constituição, ledger, atas, documentos, memberships) para que o moat seja confiança e continuidade, não lock-in artificial. Ver também [03-ORQUESTRA](03-ORQUESTRA.md) e [PRODUCTION-GATES](PRODUCTION-GATES.md) (privacidade / exportação).

**Disciplina de linguagem:** "sistema operacional autónomo" é visão arquitetural de longo prazo, não a promessa comercial do primeiro lançamento. Até o Tier 3 estar construído, o produto que vai a mercado é uma app de gestão de condomínio com IA bem desenhada — honesta sobre isso.
...
| **Enterprise** | ilimitado | 99€ | + Orquestra LLM + API | **Y1+**, só **depois de F6**; Orquestra + API |
...
**Critério de validação (não proyección):** 5 → 20 → 40 condomínios, com **CAC e win-rate medidos** em cada patamar. Só depois disto o SOM deixa de ser TBD.
...
2. **Congelar features "nice" de F4–F6** até **10 tenants pagos** ou **fim do piloto** — o que ocorrer primeiro. F4–F6 continuam como contrato arquitetural; não são prioridade de capacidade comercial.
