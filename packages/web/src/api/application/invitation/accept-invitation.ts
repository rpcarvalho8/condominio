...
const ACCEPT_WRITE_RETRIES = 6;
const acceptMutexes = new Map<string, Promise<void>>();

function isRetryableAcceptWrite(err: unknown): boolean {
  const msg = String((err as { message?: string })?.message ?? err).toLowerCase();
  return (
    msg.includes("busy") ||
    msg.includes("database is locked") ||
    msg.includes("cannot start a transaction") ||
    msg.includes("cannot commit transaction")
  );
}

async function withAcceptMutex<T>(key: string, fn: () => Promise<T>): Promise<T> {
  const prev = acceptMutexes.get(key) ?? Promise.resolve();
  let unlock: () => void = () => {};
...
async function withAcceptTransaction<T>(
  ...
  let last: unknown;
  for (let attempt = 0; attempt < ACCEPT_WRITE_RETRIES; attempt++) {
    try {
      ...
      last = err;
      if (attempt < ACCEPT_WRITE_RETRIES - 1 && isRetryableAcceptWrite(err)) {
...
export async function acceptInvitation(
  ...
  const lockKey = hashOpaqueSecret(input.token);
  return withAcceptMutex(lockKey, async () => {
    const result = await withAcceptTransaction(deps, async (txDeps) => {
