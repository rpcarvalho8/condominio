...
function mapRow(
  r: typeof bankTransactions.$inferSelect,
  fracaoNumero?: string,
) {
  ...
  return {
    ...
    fracaoNumero:      fracaoNumero ?? undefined,
    status:            r.status ?? "pending",
    requiresReview:    !!r.requiresManualReview,
  };
}

/** Resolve fração associada via quota ligada (import_ref_id). */
async function fracaoNumerosPorImportRef(
  rows: typeof bankTransactions.$inferSelect[],
  ...
  const quotaRows = await db
    .select({ id: quotas.id, numero: fracoes.numero })
    .from(quotas)
    .leftJoin(fracoes, eq(quotas.fracaoId, fracoes.id))
    .where(inArray(quotas.id, refIds));
...
export const bankMovementsRoutes = new Hono()
  ...
  .get("/condominio", requireAdmin, async (c) => {
    try {
      ...
      const rows = await db.select().from(bankTransactions).orderBy(desc(bankTransactions.date));
      const fracaoByRef = await fracaoNumerosPorImportRef(rows);

      let mapped = rows.map((r) =>
        mapRow(r, r.importRefId ? fracaoByRef.get(r.importRefId) : undefined),
      );
  ...
  .patch("/:id/classificacao", requireAdmin, async (c) => {
    try {
      ...
      const fracaoInformada = body.fracaoId?.trim().toUpperCase();

      if (quotaTipo !== null && !fracaoInformada) {
        return c.json({
          ok: false,
          error: "Selecione a fração destinatária antes de classificar como quota.",
        }, 400);
      }
      ...
      let fracaoAtribuida: string | null = null;

      if (quotaTipo !== null) {
        const fracaoNumero = fracaoInformada!;

        console.log(`[PATCH classificacao] quotaTipo=${quotaTipo} fracaoNumero=${fracaoNumero} txn.debtorName=${txn.debtorName ?? "(none)"}`);
        ...
        if (fracaoMatch) {
          fracaoAtribuida = fracaoMatch.numero;
          ...
          if (quotaId) {
            ...
            await learnPagadorPerfil({
              ...
              rubrica: rubricaFromClassificacao(classificacao),
              fonte: "manual",
            });
          }
        }
